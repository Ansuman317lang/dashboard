...
def make_chart(info, height=200, font_size=9, show_legend=False):
    ...
    # Debug output: show the first few rows of the grouped data
    st.write("Grouped data for chart:", grp.head(10))
    ...
    # If grouping results in empty data, warn and return
    if grp.empty:
        st.warning("No grouped data to plot. Check your data columns and filters.")
        return None
    ...
    y_max = grp["Count"].max() if not grp.empty else 0
    fig.update_layout(
        height=height,
